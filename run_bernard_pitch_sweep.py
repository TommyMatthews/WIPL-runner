        ...
        for pol in pol_run_dict.keys():
            for morpho in morpho_list:
                for pitch in pitch_list:
                    ...
                    if slant == 0.5:
                        BASE_FILE_NAME = f"Bernard_parameterised_{frequency_name}_{pol}_{wing_pos}_{scale}_{morpho}_p{pitch}_s05"
                    else:
                        BASE_FILE_NAME = f"Bernard_parameterised_{frequency_name}_{pol}_{wing_pos}_{scale}_{morpho}_p{pitch}_s{slant}"
                    
                    SAVE_PATH = r"C:\Users\NCAS\Documents\Tommy\Bernard_run_outputs\parameterised\\body_pitch_sweep\\"
